  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* Brand Logos Grid - Now showing original colors with proper visibility */}
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-6">
            {brandLogos.map((brand, index) => (
              <div 
                key={brand.id}
                className="flex items-center justify-center p-3 bg-white/10 rounded-xl hover:bg-white/20 transition-all duration-300 animate-slide-up"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <img 
                  src={brand.logoUrl} 
                  alt={brand.name} 
                  className="h-12 md:h-16 w-auto object-contain opacity-90 hover:opacity-100 transition-all duration-300"
                  style={{
        ...
        <div className="text-center mt-16 animate-fade-in">
          <a 
            ...
            rel="noopener noreferrer"
          >
            <button className="bg-accent hover:bg-accent-dark text-black font-medium py-3 px-8 rounded-full transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-accent/30">
              Become A Partner
            </button>
          </a>
        </div>
